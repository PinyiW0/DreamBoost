<template>
  <div @click.stop="$router.push(`/product/${getProposalID()}/payinfo`)">
    <div class="card card-shadow pt-6 pb-9 px-6 rounded-3 border-2 border-primary">
      <div
      ...
      <img class="img-fluid mb-6 w-100 rounded-3 border" :src="feedbacks.feedbackImage" alt="" />
      <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center mb-6">
        <div>
          <h4 class="mb-1 fw-bold text-primary">{{ feedbacks.feedbackTitle }}</h4>
      ...
      <div class="mb-7 py-4 px-3 border-top border-bottom border-primary-light">
        <ul class="row gy-4 list-unstyled fs-6">
          <li class="col-lg-6 d-flex align-items-center column-gap-1">
            <div class="d-flex align-items-center mb-1 rounded-circle bg-primary" style="width: 28px; height: 28px">
...
export default {
  ...
  mounted() {
    // console.log(this.singleProposal.proposalFeedbacks);
  },
  methods: {
